...
f = open("ChessSample20.pgn") # path to your database file in PGN. You can export filtered games from ChessBase, Publish it, download the Html, then download the PNG, and that's it
...
for game_idx, game in enumerate(tqdm(my_list)):
    ...
    # try:
        ...
        # check if either players has elo ratings; if not do not analyze the game
        if game.headers["BlackElo"] == "NA" and game.headers["WhiteElo"] == "NA":
            continue
